    ...
    <div class="panel panel-primary">
        ...
        <div class="panel-body">
            ...
            if ($this->sessiondata['userid']) {
                ...
                ?>
                <form action="<?php echo $this->uri->segment(3, 0); ?>" method="post" class="form-action form-inline">
                    <?php foreach ($history as $data): 
                        if ($alreadySold !== false) {
                            echo '<div class="alert alert-warning" role="alert">
                    <strong>Warning!</strong> You edit a flip what belonging to your history. If you edit your quantity upwards, you generate some items into your bank, and this sells completely will be added back to your bank.
                    </div>';
                        }
                        if (isset($updated)) {
                        ...
                        <table class = "table table-hover table-responsive" style = "vertical-align: middle; text-align: center;">
                            ...
                            <tr style="background-color: #e5ffe5;" style="text-align: center;">
                                ...
                                <td style="text-align: center;"><input type="number"
								       ...
								       required
                                                                       style="width: 125px;"/></td>
                                <td style="text-align: center;"><?= number_format($data['buyprice'] / $data['quantity']); ?> gp</td>
                            ...
                            <?php
                            if (isset($data['selldata'])) {
                                foreach ($data['selldata'] as $sellid => $selldata):
                                    ?>

                                    <tr style="background-color: #ffe5e5;" style="text-align: center;">
                                        <td style="text-align: center;"><?= $this->Logmodel->showIcon($data['itemid']); ?></td>
                                        <td style="text-align: center;" ><span id="item"><a href="http://runescape.wikia.com/wiki/Special:Search?search=<?= $data['name']; ?>&fulltext=0"><?= $data['name']; ?></a></span></td>
                                        <td style="text-align: center;"><?= $selldata['quantity']; ?></td>
                                        <td style="text-align: center;"><?= $selldata['price']; ?></td>
